    ...
    public class Transformator
    {
        ...
        public static void Import(Shape shape, Dictionary<string, object?> dict) {
            ...
            foreach (var entry in dict)
                switch(entry.Key) {
                case "rotate":
                    if (entry.Value is not string @rotate) { break; }
                    var trio = new SafeTrio(@rotate, null, null, true);
                    group.Children.Add(new RotateTransform(trio.X, trio.Y, trio.Z));
                    break;

                case "scale":
                    if (entry.Value is not string @scale) { break; }
                    var dp = new SafeDPoint(@scale, null, null, true).Point;
                    group.Children.Add(new ScaleTransform(dp.X, dp.Y));
                    break;

                case "skew":
                    if (entry.Value is not string @skew) { break; }
