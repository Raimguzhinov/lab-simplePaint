...
namespace SimplePaint.Models.Shapes
{
    public class Shape6_CompositeFigure : IShape
    {
        ...
        public Shape? Build(Mapper map)
        {
            ...
            return new Path
            {
                Name = "sn|" + Serializer.Base64Encode(@name) + "|" + Serializer.Base64Encode(@commands.Value),
        ...
        public bool Load(Mapper map, Shape shape)
        {
            ...
            map.SetProp(PName, Serializer.Base64Decode(name[1]));
            @commands.Set(Serializer.Base64Decode(name[2]));
        ...
        public Dictionary<string, object?>? Export(Shape shape)
        {
            ...
            return new()
            {
                ["name"] = Serializer.Base64Decode(name[1]),
                ["path"] = Serializer.Base64Decode(name[2]),
                ["stroke"] = @path.Stroke,
                ["fill"] = @path.Fill,
